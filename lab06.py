...
def is_time_format(s):
    ...
    #Checking hour's condition
    pos1 = s.index(':') 
    hour = s[:pos1]
    if (hour.isdigit()): #if hour is made up of digits 
        t = 'TRUE'
    else:
        return f
    
    #Checking if hour is smaller than or equal to 12
    hour = int(hour)
    
    if hour <= 12:
        t = 'TRUE'
    else: #inputs larger than 12 does not follow the 12-format
        return f

        
    # Checking minute's condition 
    posspace = s.index(' ')
    mins = s[pos1+1:posspace]
    ...
    #Checking if there is 'AM/PM' indication
